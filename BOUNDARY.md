...
| `ref/`, `proofs/kernel-conformance/`, `proofs/ref-abstain.toml` | **judge** | The reference evaluator (ADR-0015) and its seed/ledger. An evaluator is a judging instrument, not a compiler: it reads source, emits observables, and depends on no `almide-*` crate (gated) — the charter "the judge holds no compiler" is about the thing under judgment, not about the judge's own executable reading of the text. |
